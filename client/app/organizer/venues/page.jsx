  ...
  return (
    <section className="p-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-amber-700">Venues</h1>
        <button 
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {venues.map(v => (
            <div key={v.id} className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow border border-gray-100">
              <h2 className="font-bold text-lg text-gray-800 mb-1">{v.name}</h2>
              <p className="text-gray-600 mb-2">{v.city}, {v.state}</p>
              {v.address && <p className="text-sm text-gray-500 mb-3">{v.address}</p>}
